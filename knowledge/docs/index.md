> **Propósito:** Dar contexto accionable sobre The Wise Tech — Docs dentro de The Wise Tech.
> **Audiencia:** Equipos y perfiles interesados en las bases de conocimiento y lineamientos del programa.
> **Uso recomendado:** Consulta este documento cuando necesites aplicar o compartir detalles de The Wise Tech — Docs.
> **Estado:** Activo.

# The Wise Tech — Docs

Bienvenido/a. Usa el README (one-page) como mapa general y esta documentación para profundizar por secciones.

- [Principles](./principles/_index.md)
- [Personas](./personas/_index.md)
- [Guides](./guides/_index.md)
- [Playbooks](./playbooks/_index.md)
- [Scenarios](./scenarios/_index.md)
- [Roadmap](./roadmap/roadmap.md)

## Navigation map
Consulta la versión viva en el [README del repositorio](https://github.com/scanalesespinoza/the-wise-tech/blob/main/README.md#navigation-map).

## FAQ
Las preguntas frecuentes se mantienen en el [README principal](https://github.com/scanalesespinoza/the-wise-tech/blob/main/README.md#faq) para asegurar una única fuente de verdad.

---

---
¿Te fue útil este documento?
[ ] Sí  [ ] Algo  [ ] No
Deja feedback [aquí](https://github.com/scanalesespinoza/the-wise-tech/issues/new?template=feedback.yml)
---
