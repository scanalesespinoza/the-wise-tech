...
- Legacy knowledge bases located in `guides/`, `platform/`, `scenarios/`, and `docs/wise-tech/teams/` without a unifying index.
- Multiple contribution guides (`CONTRIBUTING.md`, `CONTRIBUTING.es.md`, duplicates under `docs/`).
- Automation scripts exist under `scripts/`, but there is no documentation lint to validate Markdown links.
...
- Scenario documentation duplicated between `scenarios/payments/README.md` and language folders.

**Action:** Consolidate the bilingual content into a single neutral information architecture inside `docs/`, with language-specific assets archived. Replace redundant contribution guides with a single navigable source.
...
- ADRs exist under `adr/` but lack a central pointer from navigation.
- `infra/`, `platform/`, and `scenarios/` folders contain valuable practices but are hard to discover from README.

**Action:** Create thematic sections (`principles/`, `personas/`, `guides/`, `playbooks/`, `scenarios/`, `diagrams/`) with `_index.md` entry points that link to these assets or summarize migration plans.
...
- Mixed casing and underscores across filenames (`mkdocs.yml`, `wise-tech-alignment-architecting-teams.md`).
