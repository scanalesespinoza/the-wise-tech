...
1. Run `python -m unittest discover scenarios/payments/tests` to execute
   the scenario tests. / Ejecuta `python -m unittest discover
   scenarios/payments/tests` para correr las pruebas del escenario.
2. Consume the bilingual runbooks under `docs/` to integrate the service
   into your operations. / Usa los runbooks bilingües en `docs/` para
   integrar el servicio a tus operaciones.
3. When modifying documentation ensure both languages stay in sync by
   running `python scripts/check_bilingual_parity.py`. / Al modificar
   documentación asegúrate de mantener ambos idiomas en sincronía
   ejecutando `python scripts/check_bilingual_parity.py`.
